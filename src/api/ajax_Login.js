...
ajax.interceptors.request.use(config => {
  ...
  let nonce = ''
  for (var i = 0; i < 6; i++) {
    nonce += Math.floor(Math.random() * 9 + 1)
  }
  let checksum = sha1('1a668c1425ba' + nonce + curtime)

  config = config || {}
  config.headers = config.headers || {}
  config.headers['Content-Type'] = 'application/x-www-form-urlencoded'
  config.headers['appkey'] = '54665dca31d9111b9cd67bbebfb92f3b'
  config.headers['curtime'] = curtime
  config.headers['nonce'] = nonce
  config.headers['checksum'] = checksum
  config.headers['systemauth'] = window.localStorage.getItem('token') || '1'
  config.headers['X-CSRF-TOKEN'] = window.localStorage.getItem('xToken') || '1'
  return config
})
// 响应拦截器
